    ...
    <>
      ...
      >
        <Flex alignItems="center" justifyContent="center">
          ...
          >
            {getNetworkById(data?.node.networkId ?? 1).label}
          </Text>
        </Flex>
        <Box
          display={{
      ...
      <NetworkSelectorModal
        isOpen={isOpen}
        onClose={onClose}
        initialNetwork={getNetworkById(data?.node.networkId ?? 1).value}
      />
    </>
  );
}
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <ModalContent maxW="100%" width="600px">
        <ModalBody px={16} pt={16}>
          ...
          <Select
            onChange={async (e) => {
              setIsNetworkChanged(e.target.value !== initialNetwork);
        ...
        <ModalFooter display="flex" gap={2} py={8}>
          <PillButton
            isDisabled={!isNetworkChanged || isLoading}
            height="60px"
            px={8}
            border={0}
            onClick={() => {
