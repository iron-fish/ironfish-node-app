...
export class LedgerMultiSigner extends Ledger {
  ...
  dkgGetIdentity = async (index: number, approval = false): Promise<Buffer> => {
    const response = await this.tryInstruction((app) =>
      app.dkgGetIdentity(index, approval),
    );
  ...
  dkgRound1 = async (
    ...
    return this.tryInstruction((app) =>
      app.dkgRound1(index, identities, minSigners),
    );
  };
  ...
  dkgBackupKeys = async (): Promise<Buffer> => {
    const { encryptedKeys } = await this.tryInstruction((app) =>
      app.dkgBackupKeys(),
    );
...
export const ledgerMultiSigner = new LedgerMultiSigner();
